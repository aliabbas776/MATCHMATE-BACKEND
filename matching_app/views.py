...
class CNICVerificationView(APIView):
    """
    GET: Retrieve CNIC verification status and image URLs.
    POST: Upload CNIC images, run OCR, and auto-approve/reject.
    """
    ...
    def get(self, request):
        """
        Get user's CNIC verification status and image URLs.
        """
        try:
            verification = CNICVerification.objects.get(user=request.user)
            serializer = CNICVerificationSerializer(verification)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except CNICVerification.DoesNotExist:
            return Response(
                {
                    'detail': 'No CNIC verification found. Please upload your CNIC images.',
                    'status': 'not_uploaded'
                },
                status=status.HTTP_404_NOT_FOUND
            )
